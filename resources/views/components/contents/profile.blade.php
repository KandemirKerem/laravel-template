<main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <p class="text-sm text-slate-600">My Account</p>
            <h1 class="text-3xl font-bold text-slate-900">My Profile</h1>
        </div>
        <a href="{{--route('dashboard.view')--}}" class="hover:shadow-lg transition-all duration-300 px-4 py-3 bg-soft text-base rounded-xl text-sm font-semibold hover:text-base">Go to Index</a>
    </div>
    ...
    <section class="grid md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 space-y-6">
            @unless(auth()->user()->hasVerifiedEmail())
                <div class="rounded-2xl border border-amber-100 bg-amber-50 p-6 shadow-sm flex items-start gap-4 mb-6">
                    ...
                    <div class="flex-1 space-y-1">
                        <h3 class="text-amber-900 font-semibold">Your account is not verified yet!</h3>
                        <p class="text-amber-700 text-sm leading-relaxed">
                            {{auth()->user()->name}}, Verify your email to acces all.
                        </p>
                        <div class="pt-2">
                ...
                <form method="post" action="{{route('profile.update')}}">
                    ...
                    <div class="grid sm:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-2 mb-4">
                            ...
                            <input title="U can not change your email" readonly type="email" value="{{auth()->user()->email}}" class="w-full rounded-xl border border-slate-200 px-4 py-3 focus:border-base focus:outline-none">
                        </div>
                    </div>

                    <div class="flex items-center justify-end gap-3">
                        <button class="px-4 py-3 bg-soft text-base rounded-xl text-sm font-semibold cursor-pointer hover:shadow-lg transition-all duration-300">Cancel</button>
                        <button type="submit" class="px-4 py-3 bg-base text-white rounded-xl text-sm font-semibold cursor-pointer hover:shadow-lg hover:bg-slate-700 transition-all duration-300">Save</button>
                    </div>
            ...
            <form id="update-password" method="post" action="{{route('password.update')}}">
                ...
                @method('PUT')
                <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm space-y-4">
                    <h2 class="text-lg font-semibold text-slate-900">Update your password</h2>
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div class="space-y-2">
                        ...
                        <div class="space-y-2 sm:col-span-2">
                            <label class="text-sm font-semibold text-slate-700">New password again</label>
                    ...
                    <div class="flex items-center justify-end gap-3">
                        <button class="px-4 py-3 bg-soft text-base rounded-xl text-sm font-semibold cursor-pointer hover:shadow-lg transition-all duration-300">Cancel</button>
                        <button type="submit" class="px-4 py-3 bg-base text-white rounded-xl text-sm font-semibold cursor-pointer hover:shadow-lg hover:bg-slate-700 transition-all duration-300">Update password</button>
                    </div>
                    @if (session('success'))
